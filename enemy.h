...
struct Enemy {
    ...
    void handle_event(SDL_Event& e) {
        ...
        if(!player->dead && enemy_status < ENEMY_HURT) {
    ...
    void render(SDL_Rect& camera) {
        ...
        double health_bar_width = (double)stats->hit_point/(double)stats->hit_point_max * (double)dest.w;

        health_rect = {dest.x, dest.y - 12, (int)health_bar_width, 4};
